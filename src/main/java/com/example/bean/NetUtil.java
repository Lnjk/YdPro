...
public class NetUtil {
	...
	public static void writeJSONParams(Map<String, JSON> textParams_json,
			DataOutputStream ds) throws Exception {
		Set<String> keySet = textParams_json.keySet();
		for (Iterator<String> it = keySet.iterator(); it.hasNext();) {
			String name = it.next();
			JSON value = textParams_json.get(name);
			ds.writeBytes("--" + BOUNDARY + "\r\n");
			ds.writeBytes("Content-Disposition: form-data; name=\"" + name + "\"\r\n");
			ds.writeBytes("\r\n");
			value = value;
			ds.write(value.toString().getBytes());
